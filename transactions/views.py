...
class Repair_View(TemplateView):
    ...
    def get(self,request):
        if 'q' in request.GET:
            q= request.GET["q"]
            object_list = Repair.objects.filter(
            Q(name__icontains=q) | Q(phone__icontains=q) | Q(status__icontains=q))
        else:
            object_list=Repair.objects.all().order_by('-time')
...
class Repair_detailView(DetailView):
    model = Repair
    template_name = "repair/repair_bill.html"
    context_object_name = 'bill'


class Repair_createView(CreateView):
    form_class= RepairForm
    template_name ="repair/new_repair.html"
    success_url=reverse_lazy('home')

class Repairupdate(UpdateView):
    model= Repair
    form_class=RepairupdateForm
    template_name ="repair/r_update.html"
